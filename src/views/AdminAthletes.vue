...
const fetchAthletes = async () => {
  try {
    ...
    const res = await axios.get("http://localhost:3127/tracker-t7/athletes", {
      headers: {
...
const performDelete = async () => {
  try {
    await axios.delete(`http://localhost:3127/tracker-t7/athletes/${athleteToDelete.value.id}`, {
...
const saveEdit = async () => {
  try {
    ...
    await axios.put(
      `http://localhost:3127/tracker-t7/athletes/${editedAthlete.value.id}`,
      {
    ...
    await axios.put(
      `http://localhost:3127/tracker-t7/users/${editedAthlete.value.id}`,
